...
extension PuzzlePieceX on PuzzlePiece {
  ...
  Set<Cell> relativeCells(final double unit) {
    ...
    final startRow = (bounds.top - 0) ~/ unit; // 0 is an offset if needed
    final startCol = (bounds.left - 0) ~/ unit;

    // Calculate how many grid units the bounding box spans.
    final numRows = ((bounds.bottom - bounds.top) / unit).ceil();
    final numCols = ((bounds.right - bounds.left) / unit).ceil();
    ...
    for (var r = 0; r < numRows; r++) {
      for (var c = 0; c < numCols; c++) {
        ...
        final cellCenter = Offset((startCol + c) * unit + unit / 2, (startRow + r) * unit + unit / 2);
  ...
  Set<Cell> cells(final Offset origin, final double cellSize) {
    ...
    final startCol = ((bounds.left - origin.dx) / cellSize).floor();
    final startRow = ((bounds.top - origin.dy) / cellSize).floor();
    final endCol = ((bounds.right - origin.dx) / cellSize).ceil();
    final endRow = ((bounds.bottom - origin.dy) / cellSize).ceil();
  ...
  Path getTransformedPath() => path.transform(_getMatrix().storage);
  ...
  Path get path => borderRadius > 0 ? _roundExternalCorners(originalPath, borderRadius) : originalPath;
  ...
  Path _roundExternalCorners(final Path originalPath, final double radius) {
    final vertices = _extractVertices(originalPath);
    if (vertices.isEmpty) return originalPath;
    final n = vertices.length;

    final prevLinePoints = List<Offset>.filled(n, Offset.zero);
    final nextLinePoints = List<Offset>.filled(n, Offset.zero);
    final roundFlag = List<bool>.filled(n, false);
    final arcOffsets = List<double>.filled(n, 0.0);
    ...
    for (var i = 0; i < n; i++) {
      ...
      if ((angle - math.pi).abs() < 0.01 || !isConvex) {
        ...
        roundFlag[i] = true;
        final double offset = math.min(radius, math.min(len1 / 2, len2 / 2));
        arcOffsets[i] = offset;
    ...
    final newPath = Path();
    final startPoint = roundFlag[0] ? prevLinePoints[0] : vertices[0];
    newPath.moveTo(startPoint.dx, startPoint.dy);

    for (var i = 0; i < n; i++) {
      if (roundFlag[i]) {
        newPath.lineTo(prevLinePoints[i].dx, prevLinePoints[i].dy);
        newPath.arcToPoint(nextLinePoints[i], radius: Radius.circular(arcOffsets[i]));
      } else {
  ...
  List<Offset> _extractVertices(final Path path) {
    final points = <Offset>[];
    for (final metric in path.computeMetrics()) {
      final samples = (metric.length / 2).ceil();
      Offset? lastPoint;
    ...
    return _removeCollinear(points);
  }

  List<Offset> _removeCollinear(final List<Offset> points, {final double tolerance = 0.5}) {
